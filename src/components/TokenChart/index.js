...
import { Area, Line, XAxis, YAxis, ResponsiveContainer, Tooltip, AreaChart, LineChart, BarChart, Bar } from '../../utils/rechartsCompat'
...
const TokenChart = ({ address, color, base }) => {
  ...
  // default to line chart for price view
  ...
  // compute local browser start time for last 10 days (include today)
  const localStartTime = dayjs().startOf('day').subtract(9, 'day').unix()
  // when showing Liquidity or Volume, always force last 10 days using local time;
  // otherwise use the selected timeframe for price charts
  const timeframeStart = getTimeframe(timeWindow)
  // For Liquidity/Volume: use last-10-days only when WEEK selected, otherwise respect selected timeframe
  let utcStartTime = timeframeStart
      ...
      {chartFilter === CHART_VIEW.PRICE && (
        frequency === DATA_FREQUENCY.LINE ? (
          <ResponsiveContainer aspect={below1080 ? 60 / 32 : 60 / 16}>
            <LineChart margin={{ top: 0, right: 10, bottom: 6, left: 0 }} data={chartData}>
              <XAxis
                tickLine={false}
                axisLine={false}
                interval="preserveEnd"

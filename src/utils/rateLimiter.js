    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        const isNetworkError =
          // Apollo may wrap network errors differently; check common indicators
          error && (error.networkError || error.name === 'TypeError' || msg.includes('NetworkError') || msg.includes('Failed to fetch'))

        if ((is429 || isNetworkError) && attempt < maxRetries - 1) {
          // Exponential backoff: baseDelay, 2*baseDelay, 4*baseDelay
          const delay = baseDelay * Math.pow(2, attempt)
          console.log(`Transient error, retry ${attempt + 1}/${maxRetries} after ${delay}ms`, msg)

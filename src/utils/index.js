...
export function getTimeframe(timeWindow) {
  ...
  switch (timeWindow) {
    ...
    case timeframeOptions.ALL_TIME:
      // For all-time, return epoch start (0) so components can show full history
      utcStartTime = 0
      break
    default:
      // Fallback to 1 year if an unknown option is provided
      utcStartTime =
        utcEndTime
          .subtract(1, 'year')
          .startOf('year')
...
export const toNiceDate = date => {
  // Use browser local time for date formatting
  return dayjs.unix(date).format('MMM DD')
}

export const toWeeklyDate = date => {
  // Use browser local time for week range formatting
  const formatted = dayjs.unix(date)
  ...
  return wkStart.format('MMM DD') + ' - ' + wkEnd.format('MMM DD')
}
...
export async function splitQuery(query, localClient, vars, list, skipCount = 100) {
  ...
  while (!allFound) {
    ...
    try {
      ...
      // result.data can be undefined for transient network or subgraph errors.
      const batchData = result && result.data ? result.data : {}
      fetchedData = {
        ...fetchedData,
        ...batchData
      }
      const batchKeys = Object.keys(batchData)
      // If no data returned for this batch or fewer keys than requested, stop fetching further.
      if (batchKeys.length === 0 || batchKeys.length < skipCount || skip + skipCount > list.length) {
        allFound = true
      } else {
        skip += skipCount
...
export const toNiceDateYear = date => dayjs.unix(date).format('MMMM DD, YYYY')

...
const INITIAL_STATE = {
  CURRENCY: 'USD',
  // default charts to 1 month (30 days)
  TIME_KEY: timeframeOptions.MONTH
}
...
export function useStartTimestamp() {
  ...
  useEffect(() => {
    let startTime
    if (activeWindow === timeframeOptions.ALL_TIME) {
      startTime = 0
    } else if (activeWindow === timeframeOptions.WEEK) {
      startTime = dayjs.utc().subtract(1, 'week').startOf('day').unix() - 1
    } else if (activeWindow === timeframeOptions.MONTH) {
      startTime = dayjs.utc().subtract(1, 'month').startOf('day').unix() - 1
    } else if (activeWindow === timeframeOptions.THREE_MONTHS) {
      startTime = dayjs.utc().subtract(3, 'month').startOf('day').unix() - 1
    } else if (activeWindow === timeframeOptions.YEAR) {
      startTime = dayjs.utc().subtract(1, 'year').startOf('day').unix() - 1
    } else {
      startTime = dayjs.utc().subtract(1, 'year').startOf('day').unix() - 1
    }

    // if we find a new start time less than the current starttime - update oldest point to fetch
